...
| ML-DSA rand handling | The caller-supplied `io.Reader` is honoured by `MLDSA87.New`, `MLDSA87.Sign`, `MLDSA87.SignAttached`, and `crypto.Signer.Sign`: non-nil → its bytes drive key seed or `RND_BYTES`; nil → `crypto/rand.Reader` is used. |
| Stateless | Yes |
